...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const sfLimitHour = document.getElementById('sfLimitHour');
  const sfLimitDay = document.getElementById('sfLimitDay');
  ...
  function updatePresetUI(presetName) {
    ...
    // Atualizar info box
    const sfInfo = document.getElementById('sfInfoText');
    if (sfInfo && SAFETY_PRESETS[presetName]) {
      sfInfo.textContent = SAFETY_PRESETS[presetName].info;
    }
    // Atualizar os cards de limite no display
    const p = SAFETY_PRESETS[presetName];
    if (p) {
      const hLimit = document.getElementById('sfHourlyLimit');
      const dLimit = document.getElementById('sfDailyLimitVal');
      if (hLimit) hLimit.textContent = sfLimitHour ? sfLimitHour.value : p.MAX_PER_HOUR;
      if (dLimit) dLimit.textContent = sfLimitDay ? sfLimitDay.value : p.MAX_PER_DAY;
    }
  }
  ...
  document.querySelectorAll('.safety-preset-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!preset || !SAFETY_PRESETS[preset]) return;
      ...
      sendToIg('APPLY_SAFETY_PRESET', { preset }, (r) => {
        if (sfSaveMsg) {
          ...
          sfSaveMsg.classList.add('show');
          setTimeout(() => sfSaveMsg.classList.remove('show'), 3000);
        }
      });
    });
  });
  ...
  if (sfSaveLimits) {
    sfSaveLimits.addEventListener('click', async () => {
      ...
      let warning = '';
      if (p) {
        if (hourVal > p.MAX_PER_HOUR * 1.5) warning = 'Limite por hora muito alto para este perfil!';
        if (dayVal > p.MAX_PER_DAY * 1.5) warning = 'Limite diário muito alto — risco de ban!';
      }
      ...
      const hLimit = document.getElementById('sfHourlyLimit');
      const dLimit = document.getElementById('sfDailyLimitVal');
      if (hLimit) hLimit.textContent = hourVal;
      if (dLimit) dLimit.textContent = dayVal;

      // Notificar content script para atualizar limites em tempo real
      sendToIg('UPDATE_SAFETY_LIMITS', limits, (r) => {
        if (sfSaveMsg) {
          sfSaveMsg.textContent = warning || ((r && r.ok) ? 'Limites salvos!' : 'Salvo localmente');
          sfSaveMsg.style.color = warning ? '#E17055' : '#00B894';
          sfSaveMsg.classList.add('show');
          setTimeout(() => { sfSaveMsg.classList.remove('show'); sfSaveMsg.style.color = ''; }, 3000);
        }
      });
    });
  }
  ...
  async function checkStatus() {
    ...
    try {
      ...
      chrome.tabs.sendMessage(tab.id, { type: 'GET_LOVABLE_STATUS' }, (r) => {
          ...
          if (safetySection) {
            ...
            if (sfWarmup) {
              if (sf.warmupDay > 0 && sf.warmupDay <= 7) {
                ...
                sfWarmup.textContent = 'Warmup Dia ' + sf.warmupDay + '/7';
              } else {
                sfWarmup.style.display = 'none';
              }
            }
            ...
            if (sfAlert && sfAlertText) {
              if (sf.isPaused && sf.pauseReason) {
                ...
                sfAlertText.textContent = sf.pauseReason + (sf.cooldownRemaining > 0 ? ' (' + sf.cooldownRemaining + ' min restantes)' : '');
              } else if (sf.consecutiveErrors > 2) {
                sfAlert.classList.add('visible');
                sfAlertText.textContent = sf.consecutiveErrors + ' erros consecutivos — bot pode ser pausado em breve';
              } else {
                sfAlert.classList.remove('visible');
              }
            }
        ...
        if (r.safety) {
          const sf = r.safety;
          const hPctVal = sf.hourlyLimit > 0 ? (sf.hourlyActions / sf.hourlyLimit) : 0;
          const dPctVal = sf.dailyLimit > 0 ? (sf.dailyActions / sf.dailyLimit) : 0;
          const errorPenalty = Math.min(0.3, (sf.consecutiveErrors || 0) * 0.1);
          const blockPenalty = sf.isPaused ? 0.5 : 0;
          const usage = Math.max(hPctVal, dPctVal);
          const healthScore = Math.max(0, Math.round((1 - usage - errorPenalty - blockPenalty) * 100));
          const pctStr = Math.min(100, Math.max(0, healthScore)) + '%';

          if (healthCircle && healthPct) {
            healthCircle.style.setProperty('--pct', pctStr);
            healthPct.textContent = healthScore;
            let colorClass = 'green';
            if (healthScore < 40) colorClass = 'red';
            else if (healthScore < 70) colorClass = 'yellow';
            healthCircle.className = 'health-circle ' + colorClass;
          }
          if (healthLabel) {
            if (sf.isPaused) {
              healthLabel.textContent = 'EM COOLDOWN';
              healthLabel.className = 'health-label red';
            } else if (healthScore < 40) {
              healthLabel.textContent = 'ATENCAO';
              healthLabel.className = 'health-label red';
            } else if (healthScore < 70) {
              healthLabel.textContent = 'MODERADO';
              healthLabel.className = 'health-label yellow';
            } else {
              healthLabel.textContent = 'CONTA SEGURA';
              healthLabel.className = 'health-label green';
            }
          }
          if (healthSub) {
            const parts = [];
            parts.push(`Hora: ${sf.hourlyActions}/${sf.hourlyLimit}`);
            parts.push(`Dia: ${sf.dailyActions}/${sf.dailyLimit}`);
            if (sf.consecutiveErrors > 0) parts.push(`${sf.consecutiveErrors} erros`);
